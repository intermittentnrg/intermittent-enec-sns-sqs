...
queue_url = "https://sqs.me-central-1.amazonaws.com/115154022797/enec-sns-sqs-github"
sqs = Aws::SQS::Client.new(region: 'me-central-1')
paths = []
receipt_handles = []
...
loop do
  ...
  receive_message_result.messages.each do |message|
    ...
    script =~ /let actualDate = new Date\("([^"]+)"\)/
    path = ::Regexp.last_match(1).gsub(/\s/, '/')
